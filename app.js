...
function renderCycle(all,specific,p,pres,f){
  ...
  pres.forEach((key,i)=>{
    const pm=pmeta(key);
    const angle=(-90 + (360/pres.length)*i) * Math.PI/180;
    ...
    card.style.setProperty('--seg',trioColor(key));
    card.style.position='absolute';
    card.style.left='-58px';
    card.style.top='-34px';
    card.innerHTML=`<b>${pm.label}</b><small>${pm.term}</small>`;

    const sp=specific.filter(r=>norm(r[p])===key);
    const ev=evaluatedSpecific(sp);
    const full=ev.filter(r=>outcome(r[f])==='Fulfilled').length;
    const part=ev.filter(r=>outcome(r[f])==='Partly Fulfilled').length;
    const notf=ev.filter(r=>outcome(r[f])==='Not Fulfilled').length;
...
function renderTrioLegend(specific,p,f){
  ...
  Object.entries(TRIOS).forEach(([id,t])=>{
    ...
    const d=document.createElement('div');
    d.className='trio-item';
    d.style.setProperty('--trio-color',t.color);
